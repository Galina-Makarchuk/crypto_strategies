"""Dynamic horizontal-level detection — resistance, support, pullback.

Ported from the `ema` project's ``levels.py``. Detects three families of
horizontal price levels from OHLC candles. Each family is seeded at a strict,
symmetric ``pivot_window``-bar pivot, then tracked forward until *invalidated*:

- **Resistance** — seeded at **minimum lows** (the pivot's low becomes the
  level); invalidated when a later candle's **high** comes within the tolerance,
  or the level is bracketed (``low <= level <= high``) by ``invalidation_candles``
  candles.
- **Support** — seeded at **maximum highs** (the pivot's high becomes the level);
  invalidated on a later candle's **low** within tolerance, or bracketing.
- **Pullback** — seeded at **minimum highs** OR **maximum lows**; an inside bar
  seeds two. Invalidated like support (tested on the candle **low**).
...
``start_idx`` points at the visual pivot bar, but invalidation tracking — and the
level's existence — begins only at confirmation.

This sits in the engine's detector layer, peer to :mod:`engine.swing_detector`
(stateful pivot detection returning dataclass records with a lifecycle) — *not*
in ``indicators.py`` (which is pure ``(frame, params) -> Series``). The plotting
helper lives in :mod:`engine.visualization` to keep this module plotly-free.
...
Public API: :class:`Level`, :func:`detect_resistance_levels`,
:func:`detect_support_levels`, :func:`detect_pullback_levels`,
:func:`detect_all_levels`.
"""
...
class Level:
    price: float
    start_idx: int                        # candle index where the level was seeded
    cross_count: int = 0                  # candles whose [low, high] contained the level
    invalidated_at: Optional[int] = None  # candle index where invalidation fired, or None
...
def detect_resistance_levels(
    ...
    for j in range(n):
        ...
        if confirm_idx >= pivot_window and pivots[confirm_idx]:
            new = Level(price=float(lows[confirm_idx]), start_idx=confirm_idx)
            levels.append(new)
            active.append(new)
    return levels
...
def detect_support_levels(
    ...
    for j in range(n):
        ...
        confirm_idx = j - pivot_window
        if confirm_idx >= pivot_window and pivots[confirm_idx]:
            new = Level(price=float(highs[confirm_idx]), start_idx=confirm_idx)
            levels.append(new)
            active.append(new)
    return levels
