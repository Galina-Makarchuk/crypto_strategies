...
is the only hook — and `i` is the latest known bar.  Look-ahead is structurally
...
class Backtester:
    ...
    def run(self, df: pd.DataFrame, interval: str = "15") -> BacktestResult:
        ...
        # Let strategy pre-compute indicators (on a copy)
    ...
    def _compute_stats(
        ...
        # Approximate Sharpe (trade-level, not time-scaled)
        if pnls.std() > 0:
            result.sharpe_approx = float(pnls.mean() / pnls.std())
