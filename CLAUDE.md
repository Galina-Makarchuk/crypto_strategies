...
3. **Runners** — [Backtester.run()](engine/backtester.py#L138) iterates `on_bar` across every bar; [LiveEngine._tick()](engine/live.py#L203) calls `on_bar` only on the last bar. Same strategy code runs in both modes.

### Signals, positions, and P&L flow through [core.py](engine/core.py)

`on_bar` never returns a signal — it calls `state.enter()` / `state.exit()` on a [PositionState](engine/core.py#L159). The state machine rejects invalid transitions (double-entry, exit-while-flat), which means strategy code does not need to track position status itself.
