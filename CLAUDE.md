...
2. **Strategies** ([strategies/](engine/strategies/)) — each subclasses [BaseStrategy](engine/strategies/base.py) and implements two methods:
   ...
   - `on_bar(i, df, state)` — evaluate bar `i`. **May only read `df.iloc[0..i]`**. This is the contract that prevents look-ahead; both the backtester and the live engine depend on it. It's also *enforceable*: [`Backtester.run(audit_lookahead=True)`](engine/backtester.py) feeds `on_bar` a view truncated to `[0..i]`, so a future read raises or changes results. [test_config_propagation.py](engine/tests/test_config_propagation.py) runs every strategy in audit mode and asserts the trades match a normal run.
...
- **[StrategyConfig](engine/strategy_configurator.py)** (in [strategy_configurator.py](engine/strategy_configurator.py)) — *how signals are generated*: indicator periods, multipliers, and the per-strategy structural levels the strategy still computes. Add new signal-logic knobs here. Consumed by strategies via `self.config`. (Moved out of `core.py`, which now holds only domain types + the `PositionState` state machine.) Validates its fields in `__post_init__` (like `TradingConfig`), so a bad knob — in this file, a notebook `dataclasses.replace`, or a `sweep` grid — fails loudly at construction instead of producing NaN indicators downstream. This file **also** holds the exit-policy catalog — see "Exit policies" below; the catalog is checked at import (`_validate_exit_catalog`) so `PER_STRATEGY_EXIT` must list exactly the `StrategyName` members and map only to real presets.
